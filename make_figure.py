...
def process_file_lazy(file_path, min_worth=0):
    ...
    return pl.scan_csv(file_path).filter(
...
def main(min_worth, group_label, group_desc, show):
    ...
    # Generate plots
    plot_svg(dates, wealth_columns, individual_counts, trend_data, group_label, group_desc, show)
    plot_html(dates, wealth_columns, individual_counts, trend_data, group_label, group_desc, show)
