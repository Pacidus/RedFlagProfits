...
def normalize_string_dict(value_dict, reference_dict):
    ...
    return {
        k: (reference_dict.get(v, v) if isinstance(v, str) else v)
...
def create_combined_parquet():
    """
    Create a single combined parquet file with all billionaire data.
    Use a partitioned approach for more efficient querying.
    """
    logger.info("Creating combined parquet dataset...")
    parquet_files = glob("data_parquet/*.parquet")
    if not parquet_files:
        logger.warning("No parquet files found to combine")
        return

    # Create year/month partitioned dataset
    os.makedirs("data_parquet/all_billionaires", exist_ok=True)
    ...
    for i in range(0, len(parquet_files), batch_size):
        ...
        logger.info(
            f"Processing batch {i//batch_size + 1} of {(len(parquet_files) + batch_size - 1) // batch_size}"
        )
        ...
        # Write batch to partitioned dataset
        batch_table = pa.Table.from_pandas(batch_df)
        pq.write_to_dataset(
            batch_table,
            root_path="data_parquet/all_billionaires",
